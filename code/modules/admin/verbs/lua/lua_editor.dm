		...
		while(LAZYLEN(path) > traversal_depth_offset)
			...
			switch(path_element["type"])
				...
				if("value")
					...
					to_chat(usr, "<span class='boldwarning'>invalid path element type \[[path_element["type"]]] for list traversal (expected \"key\" or \"value\"</span>")
					return
			// The element we are entering SHOULD be a list, unless we're at the end of the path
			if(!islist(path_list) && LAZYLEN(path))
				to_chat(usr, "<span class='boldwarning'>invalid path element \[[path_list]] for list traversal (expected a list)</span>")
				return
		...
		if("callFunction")
			...
			while(LAZYLEN(recursive_indices))
				...
				if(!(istext(key) || isnum(key)))
					to_chat(usr, "<span class='boldwarning'>invalid key \[[key]] for function call (expected text or num)</span>")
					return
				function += key
				if(islist(value))
					current_list = value
					...
					if(function_regex.Find(value))
						break
					to_chat(usr, "<span class='boldwarning'>invalid path element \[[value]] for function call (expected list or text matching [function_regex])</span>")
					return
...
/client/proc/open_lua_editor()
	...
	if(SSlua.initialized != TRUE)
		to_chat(usr, "<span class='boldwarning'>SSlua is not initialized!</span>")
		return
