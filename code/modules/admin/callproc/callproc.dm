...
/client/proc/callproc_blocking(list/get_retval)
	...
	switch(tgui_alert(usr,"Proc owned by something?",,list("Yes","No")))
		if("Yes")
			...
			target = value["value"]
			if(!istype(target))
				to_chat(usr, "<span class='boldwarning'>Invalid target.</span>", confidential = TRUE)
				return
		if("No")
			target = null
	...
	if(targetselected)
		if(!hascall(target, procname))
			to_chat(usr, "<span class='boldwarning'>Error: callproc(): type [target.type] has no [proctype] named [procpath].</span>", confidential = TRUE)
			return
	else
		procpath = "/[proctype]/[procname]"
		if(!text2path(procpath))
			to_chat(usr, "<span class='boldwarning'>Error: callproc(): [procpath] does not exist.</span>", confidential = TRUE)
			return
...
/proc/WrapAdminProcCall(datum/target, procname, list/arguments)
	...
	if(!is_remote_handler && current_caller && current_caller != user_identifier)
		to_chat(usr, "<span class='boldwarning'>Another set of admin called procs are still running. Try again later.</span>", confidential = TRUE)
		return
...
/client/proc/callproc_datum(datum/A as null|area|mob|obj|turf)
	...
	if(!A || !is_valid_src(A))
		to_chat(usr, "<span class='boldwarning'>Error: callproc_datum(): owner of proc no longer exists.</span>", confidential = TRUE)
		return
