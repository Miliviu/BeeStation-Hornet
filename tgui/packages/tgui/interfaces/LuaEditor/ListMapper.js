...
const UnconvertibleLuaValueRegex = RegExp('^(table|function|thread): 0x[0-9a-fA-F]+$');

export const ListMapper = (props, context) => {
  const { act } = useBackend(context);

  const { list, path, editable, name, callType, vvAct, skipNulls, collapsible, ...rest } = props;
  ...
  const ListMapperInner = (element, i) => {
    ...
    const uniquelyIndexable =
      (typeof key === 'string' && !(UnconvertibleLuaValueRegex.test(key) || RefRegex.test(key))) || typeof key === 'number';
        ...
        buttons={
          editable && (
            <>
              ...
              />
              <Button icon="window-close" color="red" tooltip="Remove" onClick={() => act('removeArg', { path: entryPath })} />
            </>
          )
        }>
        {valueNode}
  ...
  const inner = (
    <>
      ...
      {editable && <Button icon="plus" tooltip="Add" onClick={() => act('addArg', { path: path })} />}
    </>
  );

  const buttons = vvAct && list?.length > 0 && <Button icon="search" tooltip="VV List" onClick={() => vvAct(path)} />;
