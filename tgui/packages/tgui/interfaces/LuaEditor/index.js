...
export const LuaEditor = (props, context) => {
  ...
  const [modal, setModal] = useLocalState(context, 'modal', noStateYet ? 'states' : null);
  ...
  switch (activeTab) {
    case 'globals': {
      tabContent = (
        <ListMapper list={globals} skipNulls vvAct={(path) => act('vvGlobal', { indices: path })} callType="callFunction" />
      );
      break;
    }
    case 'tasks': {
  ...
  return (
    <Window width={1280} height={720}>
      <Window.Content>
        ...
        {noStateYet ? (
          <Flex width="100%" height="100%" align="center" justify="space-around">
          ...
          <Stack>
            <Stack.Item>
              <Section
                ...
                buttons={
                  <>
                    <Button.File onSelectFiles={(file) => setInput(file)} accept=".lua,.luau">
                      Import
                    </Button.File>
                    <Button onClick={() => setModal('documentation')}>Help</Button>
                  </>
                }>
                <TextArea fluid width="700px" height="590px" value={input} onInput={(_, value) => setInput(value)} />
                <Button onClick={() => act('runCode', { code: input })}>Run</Button>
              </Section>
            </Stack.Item>
            <Stack.Item grow>
              <Section fill height="95%" width="100%">
                <Tabs>
                  ...
                  <Tabs.Tab selected={activeTab === 'tasks'} onClick={() => setActiveTab('tasks')}>
                    Tasks
                  </Tabs.Tab>
                  <Tabs.Tab
